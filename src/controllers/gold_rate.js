...
const fetchGoldPriceRate = async (req, res) => {
    try {
        ...
        if (response && response.data && response.data.data) {
            ...
            const buyRateWithMargin = req.currency == "USD" ? currency(buyRate || 81.27).multiply(1.015).value : currency(buyRateInAED).multiply(1.015).value;  // 1.5% margin for buy rate in AED
            const sellRateWithMargin = req.currency == "USD" ? currency(sellRate || 80.27).multiply(1.005).value : currency(sellRateInAED).multiply(1.005).value;  // 0.5% margin for sell rate in AED
...
const fetchGoldPrice = async (req, res) => {
    try {
        ...
        if (response && response.data && response.data.data) {
            ...
            const sellRateInAED = currency(sellRate || 80.27).multiply(usdToAedRate).value;
            // Calculate prices with margin
            const usdBuyRateWithMargin = currency(buyRate || 81.27).multiply(1.015).value;  // 1.5% margin for buy rate
            const usdSellRateWithMargin = currency(sellRate || 80.27).multiply(1.005).value;  // 0.5% margin for sell rate
