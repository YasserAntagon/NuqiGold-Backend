...
const createTransaction = async (req, res) => {
    ...
    try {
        ...
        if (type === 'buy') {
            ...
            if (transaction && req.currency == "USD") {
                transaction.amount = (transaction.amount / 3.6725)
            }
...
const getTransactions = async (req, res) => {
    try {
        ...
        if (type && (type == "buy" || type == "sell" || type == "credit" || type == "sip" || type == "debit")) {
            where.type = type
        }
        if (transaction_type && (transaction_type == "gold" || transaction_type == "sip" || transaction_type == "wallet" || transaction_type == "subscription_upgrade")) {
            where.transaction_type = transaction_type
        }
        let transaction = await TransactionModel.findAll({ where: where, order: [["createdAt", "DESC"]], limit: pageSize ? Number(pageSize) : 10, offset: page ? (Number(page) - 1) * (pageSize ? Number(pageSize) : 10) : 0 })
        if (req.currency == "USD") {
